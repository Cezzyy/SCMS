...
const UsersComponent = defineAsyncComponent(() => import('../components/UserList.vue'))
...
const activeComponent = ref<string>('users')
const currentComponent = shallowRef<Component>(UsersComponent)
...
<template>
  <div :class="['h-screen flex flex-col md:flex-row overflow-hidden', darkMode ? 'dark bg-bg-alt dark:bg-gray-900' : 'bg-white']">
    ...
    >
      ...
      <div class="flex items-center justify-between h-16 px-4 border-b border-accent2 dark:border-gray-800">
        <div v-if="isSidebarOpen" class="flex items-center space-x-2">
          <span class="text-accent1 text-xl font-bold">Admin Panel</span>
        </div>
        <button
          @click="toggleSidebar"
        ...
        <div :class="[isSidebarOpen ? 'flex items-center justify-between' : 'flex justify-center']">
          <div v-if="isSidebarOpen" class="flex items-center min-w-0">
            ...
            <div class="ml-3 min-w-0">
              <p class="text-sm font-medium text-text-primary dark:text-white truncate">Super Admin</p>
              <p class="text-xs text-text-secondary dark:text-gray-400 truncate">admin</p>
            </div>
          </div>
          <button
    ...
    <div class="flex-1 flex flex-col w-full overflow-hidden">
      <!-- Top navbar - fixed -->
      <header class="sticky top-0 bg-white dark:bg-gray-900 shadow-sm z-10 h-16">
        <div class="px-3 sm:px-4 lg:px-6 py-3 md:py-4 flex items-center justify-between h-full">
          <!-- Mobile menu button -->
          <button
            @click="toggleSidebar"
      ...
      <main class="flex-1 bg-bg-alt dark:bg-gray-900 p-2 sm:p-3 md:p-4 lg:p-6 overflow-y-auto h-[calc(100vh-4rem)]">
...
/* Apply max-width to tables and scrollable containers */
:deep(table) {
  max-width: 100%;
}
